...
export class ReservationService {
  ...
  getReservations(param: string, page: number, size: number, column: string, direction: string): Observable<IReservationPage> {
    return this.http.get<any>(`${this.baseUrl}/reservation?param=${param}&page=${page}&size=${size}&sort=${column},${direction}`, { withCredentials: true });
  }
  ...
  getUserReservations(token: string,  page: number, size: number): Observable<IReservationPage> {
    return this.http.get<any>(`${this.baseUrl}/reservation/user?page=${page}&size=${size}`, { withCredentials: true });
  }
